...
class SocketNew extends Component{
	...
	constructor (props){
		super(props);
		this.state = {
			secret: this.props.onAddSecret?this.props.onAddSecret: '',
			secretcontact: '',
			...
			presentes,
			escribiendo: '',
			perfil: this.props.onAddPerfil?this.props.onAddPerfil: '',
			imagenconnect2: 1
		};
	...
	handlerCargarContactos(){
		const input = { unique: this.state.secret };
		fetch("/contacts", {
		...
		.then(response => this.handlerRegresars(response.contacts, response.activo))
		.catch(error => console.error('Error:', error));

		this.setupBeforeUnloadListener();
	}

	handlerRegresars(a, c){
		if(c.usuario != "false"){
	...
	componentWillReceiveProps(props){
		...
		if(props.onAddValor == "block" && props.onAddValido == "true"){
	...
	handlerCerrar(){
		...
		this.props.onAddQuitar('none', 'false');
	...
	handlerCInvitado(e, a, j){
		const input = { unique: this.state.secret, secretcontact: "" + j + "" };
		fetch("/mnew", {
			method: 'POST',  
			body: JSON.stringify(input),  
			headers:{
	...
	render() {
		...
		return (
			<div className="col-12 col-sm-12 col-md-12 col-lg-4 col-xl-3 chat-container sinmarging sinpadding" style={{ display: this.props.onAddValor }}>
